# -*- coding: utf-8 -*-
"""
Created on Mon Feb 16 18:46:44 2026

Author: Sergi Martínez Galindo

Ordered by distance exploration strategy of the netwrok.
"""
#%%
#libraries 
...
def update_rn(J_o,o,neighbours,k,i_s):
    """Returns the updated o matrix using the random neighbour rule. Inputs:
...
@njit
def explore_nw(o,J_o,update_rule,n_a,n_n,def_nodes,order):
...
def ground_truth_network(N,k):
    ...
    return s,J,neighbours_array,num_neighbours,isolated,G
...
def realization(N,k,r,update_rule):
    ...
    s,J,n_a,n_n,iso,network=ground_truth_network(N, k)
    ...
    if iso==False:
        ...
        #exploration
        #order
        distances=nx.single_source_shortest_path_length(network,source=0)
        dist_array=np.full(N,-1,dtype=np.int32)
        for node,dist in distances.items():
            dist_array[node]=dist
        order=np.argsort(dist_array)
        
        #simulation
        o_final=explore_nw(o,J_o,update_rule,n_a,n_n,def_nodes,order)
...
for j in range(len(k_l)):
    ...
    for i in range(10):
        r=i/20+0.05
        data_generator(1000, k, r, update_majority, 1000, "mr_BFS")
...
for j in range(len(k_l)):
    ...
    for i in range(10):
        r=i/20+0.05
        data_generator(1000, k, r, update_rn, 1000, "rn_BFS")
